    ...
    public class SocketTool
    {
                ...
                if (read > 0)
                {
                    ...
                    else if (ah.receivedLength == ah.requiredLength)//接收完一次合理的数据
                    {
                        ...
                        else
                        {
                            ...
                            System.Object msg = CustomConvert.ToObject(protoId, receiveBuffer, 4, ah.requiredLength - 4);
                            Packet package = new Packet(protoId, msg);
                            Debug.LogWarning("receive:" + msg);
        ...
        public bool SendMessage(int msgId,object msg)
        {
            ...
            else
            {
                ...
                using (MemoryStream ms = new MemoryStream())
                {
                    ...
                    ms.Write(head, 0, 4);
                    ms.Write(head, 0, 4);
                    CustomConvert.Serialize(ms, msg);
        ...
        private void SendHandler(IAsyncResult async)
        {
            ...
            try
            {
                ...
                {
                    Debug.LogWarning("send success:"+sh.sentNumber);
                }
            }
            catch(Exception e)
            {
